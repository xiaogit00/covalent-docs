...
const DocsGuidesAnalyzeData = props => {
  ...
  const guides = query.allMdx.edges
    .map(({ node }) => node)
    .filter(page => page.fields.slug.match(/^\/learn\/guides\/analyze\/.+/))
    .map(page => ({
...
class DocsGuidesAnalyzeOverview extends React.Component {
...
export default DocsGuidesAnalyzeOverview
