...
const DocsGuidesConfigureData = props => {
  ...
  const guides = query.allMdx.edges
    .map(({ node }) => node)
    .filter(page => page.fields.slug.match(/^\/learn\/guides\/configure\/.+/))
    .map(page => ({
...
class DocsGuidesConfigureOverview extends React.Component {
...
export default DocsGuidesConfigureOverview
